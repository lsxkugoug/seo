---
name: blog-generator
description: "研究、规划、撰写或审查任何网站的 Blog；支持按项目配置关键词、内容范围、历史文章与发布 API，并以内容库存、内链关系、事实和效果数据决定 new、update、merge、defer 或受控发布。适用于内容集群、工具型网站、SaaS、服务型网站和知识站；不用于未经核实的批量发文或虚构搜索数据。"
---

# Blog Generator

为任何网站寻找值得写、能服务读者且不与现有内容冲突的主题。它先研究可能性，再用内容库存决定是否应建立新 URL；库存是发布门禁，不是选题的唯一来源。可以生成主题地图、brief、提纲、开头、完整草稿、内链计划或内容审查报告。项目配置可授权“审查通过后自动发布”，但生成草稿、推送到后台和上线仍是可记录、可验证的独立动作。
...
- **接入或运行某个站点**：先读 [多项目配置与发布契约](references/project-config-and-publishing.md)，加载该项目的配置；没有有效项目配置时只做研究和草稿，不能调用发布接口。
...
1. 选择 `project_id` 并读取 `config/projects/<project_id>.json` 或仅供本机使用的 `.local.json`。配置提供站点范围、关键词、API 地址、鉴权环境变量名、历史文章响应和发布 JSON 契约。密钥只能由 `secret_env` 指向的环境变量提供，不能写入 Git、文章或日志。配置缺失、示例 URL 未替换或接口契约不完整时，不得发布。
2. 确认网站或产品、目标语言和地区、目标读者、真实能力、业务目标及用户希望得到选题、brief、写作、审查还是系统集成建议。信息不足时列出假设，继续做可验证的研究；不要虚构功能、客户、搜索量、排名或竞品私有数据。
3. 先建立一个简短的 **主题地图**：网站帮助谁，在什么情境下完成什么任务，产品或服务实际能帮助到哪一步，哪些相邻问题需要内容解释而不是产品页面。主题可来自产品事实、客户语言、公开竞品、SERP、站内搜索、支持问题、GSC 查询或关键词工具。
4. 对每个主题写出 `audience_situation`、`reader_job`、`core_answer`、`reader_takeaway`、`proof_material` 和 `boundary`。题材、产品类型和关键词工具会变，但这六项是通用的选题判断单位。
5. 主题通过业务相关性、读者需求、搜索意图、可提供的独有材料、可执行性和重复风险排序。搜索量、Keyword Difficulty、竞品流量和模型评分只能作为证据的一部分，不能代替读者任务或产品事实；不要把某个站点曾用的流量或 KD 区间当作通用阈值。
6. 再读取文章库存中的 live、archived、merged、redirected URL，以及可用正文、canonical 目标和历史内链。最终查重必须阅读最相近候选的正文，不能只比较标题或关键词。
7. 记录 `inventory_coverage`。无法证明库存完整、没有读取关键相近正文或产品/来源证据不足时，可以交付研究和 provisional brief，但 `publish_allowed` 必须为 `false`。
...
项目配置要求历史引用时，每篇可发布文章至少加入配置数量的真实、上下文相关、已发布历史文章链接；每条记录目标 `content_id`、URL、锚文本和读者需要。找不到自然相关链接时，阻断发布或改为 `needs_evidence`，不能为了满足数量插入无关链接。
...
只有项目配置明确设置 `publish.mode: auto_after_review` 时，才允许调用发布 API；调用前必须满足完整库存、通过正文级去重、审查状态 `approved`、所需历史文章引用、完整 payload、幂等键和当前库存快照。请求超时或响应不明时不得以新幂等键重试；先按原键查询或由后端确认结果。
