...
从产品文档、页面、公开评论、支持问题、销售/客服记录、站内搜索、GSC、关键词工具、SERP 和同行内容收集候选。同行研究同时看成熟竞争者的已验证主题与近期新增页面：近期页面可作为待验证需求假设，不等于它已经有流量或适合本站。趋势工具可补充新兴需求信号，仍要用 SERP、产品匹配度和可交付材料验证。每条来源记录 `source_type`、URL 或快照、时间、地区/语言和已知局限。同行页面只能作为市场和任务证据：先问读者任务是否尚未被本站服务，再决定是否有独有材料，禁止按标题、段落顺序或 FAQ 改写。
...
不要复用其他网站的固定筛选数字（例如某个 volume、traffic potential 或 KD 区间）。阈值会因市场、域名实力、内容成本、转换价值和数据来源而变；若团队设置了门槛，要将其记录为站点级、可调整的决策假设，而非本 Skill 的硬规则。

## 2.1 SERP 结构审计与写作 brief

每个要进入写作的候选都应有一次可复查的 SERP 审计。使用目标地区、语言和设备搜索主查询，查看自然结果前 10 个（广告不计入），记录：

- 检查时间、查询、市场和实际查看结果数；
- 每个结果的页面类型、显性读者任务、标题承诺、主要标题/内容块、证据或资产形式；
- 共同出现且确实帮助完成任务的内容块；
- 结果之间的差异、读者没有被充分服务的问题，以及本站可以用什么真实材料补足；
- 工具给出的 intent 与实际 SERP 不一致时的判断和理由。

把审计变成 brief，而不是“模仿前十”：提纲要覆盖共同的必要问题，同时明确本站的 `unique_contribution`、不覆盖的边界和每个内容块的读者产物。不能把前十标题凑成目录，也不能因为多数页面有某段就加入与当前任务无关的背景。

推荐在 brief 中为主要部分维护下表：

| 子问题 | 当前页回答 | 证据/例子 | 呈现形式 | 完成信号 |
| --- | --- | --- | --- | --- |
| 读者在这里需要判断什么？ | 可直接复述的答案 | 来源、截图、示例、步骤或反例 | 段落、步骤、表格、图或视频 | 读者能够执行或作出决定 |

图片、GIF、截图、表格或嵌入式工具只有在能解释步骤、状态、对比或证据时使用；为每个非装饰图片提供准确替代文本。媒体数量和视觉美化不是排名保证。
